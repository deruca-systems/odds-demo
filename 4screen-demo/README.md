...
**決着（2026-07-15）**: **フロント側で解決する**旨を芥川様へ伝達済み・芥川様からの引継ぎにも記載ありのため、本項はクローズ。
方針は **5/18 サテライト石狩 現地確認打合せ 議事録 §1.3** で確定:

- 地方競馬には同名騎手識別のための**公式3文字略称体系**があるが、**単純な頭3文字切りでは再現できない**（例: 「五十嵐〇〇」を「五十嵐」で切ると同名とかぶるため公式は別の略し方をする）
- **JRA 側には3文字略称のデータが存在しない**（「地方競馬の3文字略称はJRAには使用しない」）
- → **略称は採用せず、JRA・地方とも騎手名の切り捨てで統一**する

**実装**: `common.js` の `truncateJockey(name)`。連続スペースを1つに正規化してから `length >= 5` なら先頭4文字に切る。
`single-screen.html` と `entries-results-3r.html`（成績・出走表の両モード）で共通に使用する。

- 2026-07-16 修正1: **スペースを完全除去してから4文字判定**（及川決定）。NAR データは姓名をスペースで詰めて桁揃えするため
  （例「藤江　渉」「佐々木 世麗」）、スペースを温存すると4文字枠を1つ食い、**姓3文字の騎手は名が丸ごと消え**、
  さらに**同姓騎手が区別できなくなる**（5/18 議事録「五十嵐で切っちゃうとかぶっちゃう」が実際に発生していた）。
  実データ **219名**（2026-07-13〜16・全15場）で検証:
    - **208名（95%）がフルネーム表示**になる（山口 達弥→山口達弥／藤江　渉→藤江渉）
    - **同姓衝突 2件 → 0件**（佐々木 世麗/志音→佐々木世/佐々木志、山本 聡哉/聡紀→山本聡哉/山本聡紀）
- 2026-07-15 修正2: **`entries-results-3r.html` へ適用**。従来 `single-screen.html` でしか呼ばれておらず、
  出走成績では騎手名処理が抜けていたため5文字姓名が**2行に折り返していた**（STG 実データ「菅原 吏久人」「七夕 裕次郎」で実測）。
