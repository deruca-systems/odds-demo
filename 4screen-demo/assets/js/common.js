...
// ---- 共通定数 ----
// C-01 : 本番デフォルト30秒（全画面リスト v1.0【3/10確定事項】「オッズ画面更新頻度：30秒」）。
...
//   ★要確認: 次回MTGで正式議事録化予定（ 時点では担当の決定権で確定済）。
...
// CUT-003 (サイネージカットイン) 表示秒数。要件定義書 v3.3 §3.2「締切後30秒程度」。
...
// C-01 : fast=1 連動（デモ用モード：ポーリングも10秒に短縮）。
...
(function applyCutinQuery() {
  try {
    ...
    // CUT-003 (サイネージカットイン) 秒数の URL クエリ上書き
...
window.addEventListener('message', function(e) {
  ...
  // 案B : 裏で先読み中の iframe はカットインを出してはいけない。
...
// field-rename-v0.5 : v0.5 §4.3 weather_cd (INT 1-6) → アイコンファイル名。
...
// field-rename-v0.5 : v0.5 §4.3 track_cd / track_cond_cd / course_direction
//   INT コード → 表示文字列（renderRaceHeader で使用）
//   付録A.8-A.10 準拠。
var TRACK_LABEL = { 0: 'ダ', 1: '芝', 2: 'サ', 3: '障' };
var TRACK_COND_LABEL = { 1: '良', 2: '稍重', 3: '重', 4: '不良' };
//  Phase 3: ばんえいは直線固定（course_direction=0 を"直線"として受ける）
var COURSE_DIRECTION_LABEL = { 0: '直線', 1: '右', 2: '左', 3: '直線' };

//  Phase 3: ばんえい判定（organizer_type=NAR + place_cd=03、帯広固定）
...
//   999999.9  : 3連単(JRA)
// cap 値超過時は cap 値をそのまま表示（ユーザー確定仕様）
var ODDS_CAPS = {
  NAR: {
...
// 桁数によるフォント縮小クラス。
//   デザイン「システム組み込み申し送り仕様書」§6.7.1-D（keiba-odds 28a1aeb / 8-17 push）。
//     整数部 1〜3桁 → クラスなし
//     整数部 4桁    → odds-long
...
// O-3b 追補 ( 担当指摘): min - max 形式のセル（複勝・ワイド）を組み立てる。
...
function fillMinMaxCell(cellEl, min, max, betType, organizer) {
  ...
  // 桁数クラス（§6.7.1-D）は**数値 span のみ**に付ける。区切りの '-' には付けない。
...
//                    （地全協 keiba.go.jp 出馬表と同じ「未計量は何も出さない」流儀。決定 ）
...
//  session3 修正: クラス名をデザイン design.zip 準拠の `frame-umatan` に統一
...
function renderRaceHeader(doc, race, opts) {
  ...
  if (mode === 'full') {
    ...
    if (icon) {
      // field-rename-v0.5 : race.weather → race.weather_cd (INT)
      // 🔴 **同じ値でも代入すると再取得が走る。**
    ...
    // field-rename-v0.5 : race.condition → race.track_cond_cd、race.surface → race.track_cd、
    ...
    if (raceTime) {
      ...
      // デザイン CSS (.race-time.start { background: #0D1117 }) に合わせて、発走時刻表示
...
// server_time-fix-v3 : 時刻補正ソースを HTTP Date+Age ヘッダに刷新。
...
//   （ 大井5R・6R、07-28 名古屋2R・3R、金沢4R・5R で実発生）
...
// 前日発売の参照先が存在しないときの案内（検証 実機検証 #2 / 決定）。
...
//   表示日=翌日で保存できない」バリデーションを入れること（管理画面領域）。
...
function startResilientPolling(opts) {
  ...
  async function tick() {
    ...
    try {
      ...
      // 🔴 初回描画が済んだ。遮蔽を解き、親に「入れ替えてよい」を通知する。
      notifyFirstPaint();
    } catch (err) {
      failCount++;
...
function ensureCutinTemplate() {
  ...
  // cutin.html 構造をデザイン closing-timer-board に移植。
...
function checkCutin(race, correctedNowMs) {
  ...
  // field-rename-v0.5 : race.org/venue/race_no → organizer_type/place_name/rr
  var raceKey    = (race.organizer_type || '') + '-' + (race.place_name || '') + (race.rr || '');

  // 🔴 CUT-000（ 決定）: **画面に出てから起きた変化だけ出す。**
  ...
  //   実測 : 同じ画面のヘッダが「締切2分前」、カットインが「締切 5 分前」。
  ...
  if (remainMs <= 0 && !cutinState.shownKeys[raceKey + '_closed']) {
    ...
    notifyCutinState();   //  №21: 発火済みを親へ（リロードを跨いで保持させる）
    return;
  }
 ...
 * type='signage' (CUT-003) 追加。表示時間は SIGNAGE_CUTIN_DISPLAY_SEC。
 *
...
function showCutin(type, race, minutesLeft) {
  ...
  if (type === 'signage') {
    ...
    notifyCutinState();   //  №21: 表示中はページ送りを保留させる
  ...
  // CUT-001 / CUT-002: closing-timer-board (デザイン screen6 正式版)
  // field-rename-v0.5 : race.venue → race.place_name、race.race_no → race.rr
...
window.OddsDemo = {
  ...
  joinClass: joinClass,
  setOddsLen: setOddsLen,           //  オッズ欄の文字数を --odds-len で渡す
  ...
  //  №22: オッズ画面のページ定義（描画パラメータ＋空判定＋親通知）
  //  キーをテンプレ名 → ページ番号（1始まり）へ変更
 ...
 *
 *  (売場 №22): dp20/30 の自動ページングで 2 ページ目が真っ黒になっていた。
 ...
 *       （ の録画をフレーム解析して実測。輝度 80.4 → 47.1 → 79.4 の10フレーム）。
 ...
 * dp40（人気順の自動ページング）も親が template を差し替える＝iframe を
 *   リロードする作りなので、№21（カットイン再発火）も №22（中身の無いページへ送る）も
 *   同じ穴がある。dp20/30 と同じ仕組みに載せる（担当指示）。
 */
/*  MATRIX_PAGES と同じくページ番号キーへ移した（理由は上記）。 */
var POPULAR_PAGES = [
...
/**
 *  (売場 №22): オッズ画面の各ページの「中身の有無」を親へ通知する。
...
function renderMatrixTable(container, horses, matrix, opts) {
  ...
  // O-3 : 「表示範囲の昇順上位N」による人気判定は廃止。
  //   配色は oddsColorClass（桁数ベース）がセル単位で決めるため、事前集計は不要。

  //  №22: 軸・相手の列挙は matrixPagePairs に一本化した。
  ...
  for (var pi = 0; pi < pairs.length; pi++) {
    ...
    for (var oi = 0; oi < opp.length; oi++) {
      ...
      if (type === 'umatan' && bb === axis) {
        ...
        //  Phase 5: type (umaren/umatan) + organizer_type で cap 適用
...
function startHeaderTicker(getState) {
  setInterval(function() {
    try {
      ...
      // 🔴 カットインの判定も**ヘッダーと同じ毎秒・同じ時刻基準**で行う。
      ...
      //   状態だった（ 担当様報告「締切カットインは次の画面種類に移るまで出ない」）。
